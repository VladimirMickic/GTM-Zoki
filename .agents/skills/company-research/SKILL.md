...
## Self-improvement
If, while running a process, you get corrected on a query, a `stop if`/`do not search`
condition, or any other instruction in this file or a referenced process file — edit it to
bake the correction in before you finish. A correction that isn't written back just recurs
next time, including in sessions with no memory of this one (this skill is mirrored for
both Claude Code and Codex, each with its own real copy at `.claude/skills/company-research`
and `.agents/skills/company-research` — check whether a correction applies to both copies or
is specific to the tool-name wording that intentionally differs between them).
